SeedID = 15757 -- id seed
MagBG = 564 -- id bg
TotalPTHT = 1
PlantY = 192 -- plant position
-- mode ptht
MirrorPlant = false
VerticalPlant = true
UseMRAY = true -- pake mray/no
UseUWS = false -- pake uws/no
...
{"embeds": [{
"author": {"name": "PTHT by Sekrip Cepees | ANDROID & PC",
...
"color": ]] .. math.random(0, 16777215) .. [[
          ...
          SendPacket(2, [[
action|input
text|`c]] .. PTHT .. " `bPTHT DONE `c#Sekrip Cepees")
        end
      end
    end
  end  
...
invitedWorld|0]])
  ...
  function dialog(teks)
    ...
    Var1 = [[
add_label_with_icon|big|`cPTHT by Sekrip Cepees  ||7064|
add_spacer|small|
...
||
add_spacer|small|
add_url_button||`eSaluran|NOFLAGS|https://whatsapp.com/channel/0029Vazh2wZD8SDozKXjIe1y/|Join Saluran Sekrip Cepees untuk script gratis lainnya|0|0|
add_quick_exit|]]
    if NewVersionBothax then
      SendVariantList({
        [0] = Var0,
      ...
      if type(TotalPTHT) == "number" then
        repeat
          ...
          if PTHT == TotalPTHT then
            LogToConsole("`^PTHT " .. PTHT .. "X IS DONE! `c#Sekrip Cepees")
            Sleep(820)
            SendPacket(2, [[
action|input
...
invitedWorld|0]])
  ...
  error("Powered by Sekrip Cepees")
